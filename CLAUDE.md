...
- **Hook:** `species_pipeline.py --tabllm` runs the curve per species after the canonical run (non-breaking; `run_species` is untouched). Headline config = spatial-block CV + prior-free prompt + `georegion_blind` (the honest analogue of the geography-dominance finding). Per-species/pooled figure folders use their own numbering; TabLLM figures `21`-`22` live under `figures/<species>/`.

### Score read-out: `--scoring verbalized` vs `--scoring logprob` (added 2026-07-29)

**The read-out, not the model, was the binding constraint on AUC.** Diagnosed by replaying all 39,277 cached Qwen2.5-72B responses offline (the replay reproduces `tabllm_results.json` exactly, so the decomposition is trustworthy):

| measure | value | meaning |
|---|---|---|
| mean AUC over the 12 published configs | 0.550 | the headline |
| tied positive/negative pairs | 26.2% | model gave the *identical* score to a high and a low cell |
| hard tie ceiling | **0.681** | max AUC reachable *even if every tie broke perfectly* |
| untied ranking skill | 0.568 | AUC among pairs the model *did* separate |
| AUC from the binary label alone | 0.533 | so verbalized `p_high` is worth only **+0.017** over a bare label |

Cause: LLMs emit round numbers. Across 39k calls there were only **66 distinct scores**, with 64% of the mass in the top five (`0.35` alone = 18.6%). That clustering *is* the tie mass.

- **`verbalized`** (default, back-compatible) — the model writes `{label, p_high}` under guided decoding. Keep for the Anthropic backend, which cannot do the alternative.
- **`logprob`** (recommended) — score = `P("high") / (P("high") + P("low"))` read from the answer-token distribution. This is what **TabLLM (Hegselmann et al.) actually specifies** — the LM's likelihood over the label verbalizers — so the previous implementation was a weaker variant of the method the paper cites. Continuous → the tie ceiling disappears. **Requires an OpenAI-compatible `--base-url`; the Anthropic Messages API does not return logprobs** (the client raises if you ask for it without one). Guided decoding is deliberately *off* in this mode — it would renormalize the very distribution being measured. The prompt asks for the bare word `HIGH`/`LOW`, few-shot demos become that bare word, and `max_tokens` is clamped to 8. `_p_from_logprobs` scans up to 8 content positions so leading whitespace/newline tokens don't break it, and prefix-matches tokens so tokenizer splits (`hi`+`gh`) and ` High`/`"low` variants all resolve.
- **The serialized evidence is identical across both modes** — only the answer-format instruction differs (asserted in `--selftest`), so the two runs are directly comparable.

**Fixed at the same time — `keep_reasoning` was inert.** `OUT_SCHEMA` declares an optional `reasoning` field and the docstring claimed it was "dropped in the bulk pass", but the flag was only ever stored and hashed, never used to strip it. Under guided decoding the model opened with `reasoning` (81.9% of failures), blew past `max_tokens=128`, truncated into invalid JSON, and `_parse` swallowed the `ValueError` and returned a **confident `p_high=0.0`** — 5,418 / 39,277 responses (13.8%) — which was then cached permanently. The circuit-breaker cannot catch this because a truncated response is a *successful* HTTP call. Now: `BULK_SCHEMA` omits `reasoning`, parse failures carry `ok=False` and are **never cached** (a re-run retries them) and count toward `error_abort`, `_aggregate` ignores failed votes instead of averaging fabricated zeros, and `run_curve` **drops** no-answer cells from the AUC (logged as `n_no_answer`) rather than scoring them 0.0 — with the tabular baselines scored on the identical usable subset so the comparison can't drift.

> **Cache consequence:** the cache key now hashes the schema *actually sent*, so the 39,277 existing entries are **orphaned for new verbalized runs** — deliberate, since they were produced by the buggy request and 13.8% are poisoned. The file is untouched and the 72B baseline is preserved in `tabllm_results_qwen72b.json` / `21_*_qwen72b.png` / `22_*_qwen72b.png`. Any new run rebuilds the cache; budget for it.

**Honest caveat to keep in the writeup:** untied ranking skill is only 0.568, so part of the gap is genuine weakness in reasoning from lat/lon/retention/speed/vorticity/coast-distance to habitat use, not just read-out coarseness. Expect logprob scoring to land in ~0.62–0.70; the supervised RF ceiling (0.988 random / 0.886 spatial) stays out of reach for a prompt-only method. That is consistent with the existing geography-dominance ablation and is a legitimate finding — it just deserves measuring with an uncapped scorer before it goes in the paper.
